...
SQL;
    ...
    {
        $user_data = $this->request->get('ext');
...
//        file_put_contents('id68callback.log', $this->request->get());
        ...
        $car_owner_source = $user_data[1];
        $car_owner_id = $user_data[2];
        $caller = $this->request->get('phone');
        $called = $this->request->get('call');
        $start_time = $this->request->get('start_time');
        $end_time = $this->request->get('end_time');
        $last_time = $this->request->get('last_time');
        $fee = $this->request->get('fee');
        $bye_type = $this->request->get('endtype');
...
SQL;
            $add_order_track_bind = array(
                ...
                'result' => $bye_type != 2 ? 'success' : 'failed'
            );
            $add_order_track_success = ModelEx::nativeExecute($add_order_track_sql, $add_order_track_bind);
            if(!$add_order_track_success)
            {
            ...
            $add_call_record_sql = <<<SQL
        insert into MC_CallRecord (order_id, caller, called, start_time, end_time, last_time, bye_type) values (:order_id, :caller, :called, :start_time, :end_time, :last_time, :bye_type)
SQL;
            $add_call_record_bind = array(
                'order_id' => $order_id,
                'caller' => $caller,
...
SQL;
     ...
     * 轻码云双向回呼(通话结束回调)
     */
    public function qmyCallbackAction()
    {
        $data = $this->request->getJsonRawBody(true);
        file_put_contents('qmyCallback.log', var_export($data, 1).PHP_EOL);
