...
class REST {
	...
	function __construct($ServerIP,$ServerPort,$SoftVersion)	
	{
        ...
        $this->Handle = fopen(dirname(__FILE__).'/'.$this->Filename, 'a');
	}
    ...
    */
	  ...
	  {   
        ...
        if($this->BodyType=="json"){
           ...
           'needRecord':'$needRecord','countDownTime':'$$countDownTime','countDownPrompt':'$countDownPrompt', 'cbContenType': '$this->BodyType'}";
        }else{
           $body= "<CallBack>
                     <from>$from</from>
                     <to>$to</to>
					 ...
					 <cbContenType>$this->BodyType</cbContenType>
                   </CallBack>";
        }
        $this->showlog("request body = ".$body);
        // 大写的sig参数  
